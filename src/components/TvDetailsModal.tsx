...
export default function TvDetailsModal({ show, onClose }: Props) {
  ...
  const [watchEp, setWatchEp] = useState<{ season: number; episode: number } | null>(null);
  const iframeRef                           = useRef<HTMLIFrameElement>(null);

  useEffect(() => {
    fetchJson(`/api/getTvDetailsEnhanced?id=${show.id}`)
      .then(res => {
        const createdByEntry = res.data?.created_by?.[0] ?? null;
        const seasons: Season[] = (res.data?.seasons ?? []).filter((s: Season) => s.episode_count > 0);
        setData({
          logo:           res.logo ?? null,
          trailerKey:     res.trailerKey ?? null,
          episodeRuntime: res.data?.episode_run_time?.[0] ?? null,
      ...
      .catch(() => setData({ logo: null, trailerKey: null, episodeRuntime: null, cast: [], createdBy: null, createdById: null, seasons: [] }));
  }, [show.id]);

  useEffect(() => {
    fetchJson(`/api/getTvRecommendations?id=${show.id}`)
  ...
  const genres = show.genre_ids.slice(0, 4).map(id => ({ id, name: TV_GENRE_MAP[id] })).filter(g => g.name);
  ...
  const watchSeasons = data?.seasons.filter(s => s.season_number >= 1) ?? [];
  ...
  return (
    <>
          ...
          >
              ...
              <div className="absolute bottom-0 left-0 right-0 px-7 pb-7 md:px-10 md:pb-8">
                ...
                <div className="flex items-center gap-2.5">
                  <button
                    ...
                    disabled={watchSeasons.length === 0}
                    className="flex h-10 items-center gap-[7px] rounded-full bg-white px-6 font-manrope text-[15px] font-bold tracking-[0.01em] text-black transition-all duration-200 hover:bg-white/90 hover:shadow-[0_0_32px_rgba(255,255,255,0.22)] disabled:opacity-40"
                  >
            ...
            <div className="px-7 pb-10 pt-6 md:px-10 md:pb-12 md:pt-7">
              ...
              <div className="mb-6 flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
                <span className="font-space text-[14px] font-semibold text-[#4ade80]">★ {show.vote_average.toFixed(1)}</span>
                {year && (
                  <>
                ...
                {data && data.seasons.length > 0 && (
                  <>
                    <span className="text-white/[0.18]">·</span>
                    <span className="font-space text-[14px] text-neutral-400">
                      {watchSeasons.length} Season{watchSeasons.length !== 1 ? "s" : ""}
                    </span>
                  </>
                )}
              ...
              {/* Overview */}
              <p className="mb-6 text-[15px] leading-[1.85] text-neutral-300">{show.overview}</p>

              {/* Creator / Starring / Genres */}
              <div className="mb-7 flex flex-col gap-3.5">
                ...
                {data?.createdBy && (
                  <div className="flex items-start gap-4">
                    <span className="w-[76px] shrink-0 pt-[5px] font-manrope text-[10px] font-bold uppercase tracking-widest text-neutral-600">Creator</span>
                    <p className="leading-relaxed text-[15px] text-neutral-300">
                      {data.createdById
                        ? <Link href={`/person/${data.createdById}`} onClick={onClose} className="transition-all duration-200 hover:text-white hover:underline hover:decoration-white/30 underline-offset-4">{data.createdBy}</Link>
                      ...
                      >
                        {r.poster_path
                          ? <img
                            ...
                            />
                          : <div className="flex h-full w-full items-center justify-center font-manrope text-[11px] text-neutral-600">{r.name[0]}</div>
                        }
      ...
      {showWatchModal && data && watchSeasons.length > 0 && (
        <TvWatchModal
          showId={show.id}
          showName={show.name}
          ...
          year={year ?? null}
          initialSeason={watchEp?.season}
          initialEpisode={watchEp?.episode}
          onClose={() => { setShowWatchModal(false); setWatchEp(null); }}
        />
      )}
    </>
  );
