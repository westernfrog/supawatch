...
export default function TvPageHero({
  ...
  useEffect(() => {
    fetchJson(`/api/getWatchProviders?id=${showId}&region=${region}&media_type=tv`)
      .then((res) => {
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            {episodeRuntime && (
              <>
                <span className="text-white/[0.22]">•</span>
                <span className="font-space">{episodeRuntime}</span>
              </>
            )}
            {status && (
              <>
          ...
          <div className="flex items-center gap-5 pb-7 pt-2">
            ...
            {seasonCount > 0 && (
              <span className="font-manrope text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-500">
                {seasonCount} Season{seasonCount !== 1 ? "s" : ""}
              </span>
            )}
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 px-5 pb-6 md:px-8 md:pb-8">
            ...
            {status && (
              <div
                className={cn(
                  "mb-4 inline-flex items-center gap-2 rounded-full border px-3 py-1 backdrop-blur-md",
                  status === "Returning Series"
                    ? "border-[#4ade80]/20 bg-[#4ade80]/[0.05]"
                ...
                <span className="relative flex h-1.5 w-1.5">
                  ...
                  <span
                    className={cn(
                      ...
                      status === "Returning Series" ? "bg-[#4ade80]" : "bg-neutral-500"
                    )}
                  ></span>
                </span>
                <span
                  className={cn(
                    "font-manrope text-[9px] font-bold uppercase tracking-[0.25em]",
                    status === "Returning Series" ? "text-[#4ade80]" : "text-neutral-400"
                  )}
                >
                  {status}
                </span>
            ...
            <div className="flex items-center gap-3">
              ...
              {tSrc && (
                <>
                  {!showVideo ? (
                    <div className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full border border-white/[0.22] bg-black/30">
                      <svg className="h-[22px] w-[22px] -rotate-90" viewBox="0 0 36 36">
                        <circle cx="18" cy="18" r="15" fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="2" />
                        <circle
                          cx="18" cy="18" r="15" fill="none" stroke="var(--color-brand, #e50914)" strokeWidth="2.5"
      ...
      {showWatch && watchSeasons.length > 0 && (
        <TvWatchModal
          showId={showId}
          showName={showName}
