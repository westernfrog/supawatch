...
import { Play, Volume2, VolumeX, ChevronDown, Info } from "lucide-react";
  ...
  return (
    <>
        ...
        <DialogContent className="lg:max-w-6xl max-lg:max-w-none p-0 border-none bg-[#141414] rounded-lg shadow-2xl max-h-[90vh]">
          ...
          <div className="relative">
            <div className="relative h-[40vh] lg:h-[60vh] overflow-hidden rounded-t-lg">
              ...
              <div className="absolute inset-0 bg-linear-to-r from-[#141414]/60 via-transparent to-transparent" />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 lg:p-8 space-y-3 lg:space-y-4">
                ...
                {logo ? (
                  ...
                  <div className="h-10 w-44 lg:h-16 lg:w-72 animate-pulse rounded-lg bg-white/10" />
                ) : (
                ...
                <div className="flex items-center gap-3">
                  {/* Watch Now — plays directly */}
                  <button
                    onClick={handleWatchNow}
                    className="inline-flex gap-2 items-center px-5 lg:px-10 py-2 lg:py-3 bg-white text-black rounded font-semibold hover:bg-white/90 transition-all hover:scale-105 shadow-lg text-sm lg:text-base active:scale-95"
                  >
              ...
              <div className="lg:col-span-1 space-y-3 text-sm">
                ...
                {credits?.data?.crew?.find((c) => c.job === "Director") && (
                  <div>
                    ...
                    <Link
                      href={`/person/${credits.data.crew.find((c) => c.job === "Director").id}`}
                      onClick={() => {
                        handleNavigationClick();
                        onOpenChange(false);
                      }}
                  ...
                  {loadingEpisodes ? (
                    [...Array(3)].map((_, i) => (
                      ...
                      >
                        ...
                        <div className="w-36 lg:w-56 h-20 lg:h-28 bg-white/10 rounded shrink-0" />
                    ...
                    episodes.map((episode) => (
                      <div
                        key={episode.id}
                        onClick={() => setPlayEpisode(episode)}
                        className="flex items-center gap-3 lg:gap-6 px-4 lg:px-6 py-3 rounded hover:bg-white/8 transition-all group cursor-pointer border-b border-white/10 last:border-0"
                      >
                        <div className="text-xl lg:text-2xl font-semibold text-white/60 w-6 lg:w-8 text-center shrink-0 group-hover:text-white transition-colors">
                        ...
                        {episode.runtime && (
                          <div className="text-sm lg:text-base text-white/60 font-medium shrink-0 w-12 lg:w-16 text-right">
                            {episode.runtime}m
                          </div>
                        )}
                      </div>
      ...
      {playEpisode && (
        <Dialog
          open={!!playEpisode}
          onOpenChange={(o) => !o && setPlayEpisode(null)}
        >
          <DialogContent className="lg:max-w-[82vw]! lg:h-[88vh]! w-full h-[55vw] p-0 border-none bg-black">
            <DialogTitle className="sr-only">
              {getTitle(item)} — S
              {String(playEpisode.season_number).padStart(2, "0")}E
              {String(playEpisode.episode_number).padStart(2, "0")}
            </DialogTitle>
            <iframe
              className="w-full h-full rounded-md"
              src={`https://vidsrcme.ru/embed/tv?tmdb=${item.id}&season=${playEpisode.season_number}&episode=${playEpisode.episode_number}`}
              allowFullScreen
            />
          </DialogContent>
        </Dialog>
